...
export async function POST(request: NextRequest) {
  try {
    ...
    let goalsContext = '';
    let nativeSpeciesData: Array<{ common_name: string; scientific_name: string; layer: string; mature_height_ft: number; sun_requirements?: string; water_requirements?: string }> = [];
    let ragContext = '';
      ...
      if (farm.climate_zone) {
        ...
        if (nativeResult.rows.length > 0) {
          nativeSpeciesData = nativeResult.rows.map((s: any) => ({
            ...
            sun_requirements: (s.sun_requirements as string) || undefined,
            water_requirements: (s.water_requirements as string) || undefined,
          }));
        }
      }
