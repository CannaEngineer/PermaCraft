...
async function fetchProfile(id: string, viewerId: string | null) {
  try {
    ...
    // Parallel fetch all profile data — each wrapped for resilience
    ...
    return {
      ...
      follower_count: (followerResult.rows[0] as any)?.count || 0,
      following_count: (followingResult.rows[0] as any)?.count || 0,
...
async function fetchUserFarms(userId: string) {
  try {
    ...
    console.error('Failed to fetch user farms:', error);
    return [];
  }
}
...
export default async function ProfilePage({ params }: PageProps) {
  ...
  let profile, postsData, farms;
  try {
    [profile, postsData, farms] = await Promise.all([
